...
router.get('/devices/:device_id/hist/fahrenheit', function (req, res) {
    ...
    db.collection('hist_fahrenheit').find({coreid: req.params.device_id, time: {$gte: new Date(startDate)}}, {sort: {time: 1}}).toArray(function (err, items) {
        res.json(items);
    });
})
...
router.get('/weather/:station_id/fahrenheit', function (req, res) {
    var db = req.db;
    var startDate = moment().subtract(14, 'days').startOf('day');
    db.collection('wunderground_aggregate').find({station_id: req.params.station_id}, {sort: {time: 1}}).toArray(function (err, items) {
        res.json(items);
    });
});
